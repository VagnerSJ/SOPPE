...
// -- Using MPI to access multiple GPUs to simulate the x-ray projection image:
  ...
  { 
      ...
      printf(    "     ***                                                                       ***\n");
      printf(    "     ***                                SOPPE                                  ***\n");
      printf(    "     ***                 Software de Paralelização do Penelope                 ***\n");
      printf(    "     ***              version 1.0 (https://github.com/VagnerSJ/SOPPE)          ***\n");
      printf(    "     ***                                                                       ***\n");
      printf(    "     *****************************************************************************\n\n");
...
///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    char file_name_output_num_p[264];
...
//     printf("       [Input] %s",new_line);
  ...
  // -- Init. [SECTION DOSE DEPOSITION v.2012-12-12] (SOPPE v.1.0):
...
void load_voxels(int myID, char* file_name_voxels, float* density_max, struct voxel_struct* voxel_data, float2** voxel_mat_dens_ptr, unsigned int* voxel_mat_dens_bytes, short int* dose_ROI_x_max, short int* dose_ROI_y_max, short int* dose_ROI_z_max)
{
  ...
  MASTER_THREAD 
  {
    ...
    if (strstr(file_name_voxels,".gz")==NULL)
      printf("         (note that SOPPE can also read voxel and material files compressed with gzip)\n");     // !!zlib!!  
    fflush(stdout);
  }
  do
  {    
        ...
        ulonglong2* materials_dose, ulonglong2** materials_dose_device, int flag_material_dose, int num_projections)
{
  ...
  int GPU_cores = _ConvertSMVer2Cores(deviceProp.major, deviceProp.minor) * deviceProp.multiProcessorCount;    // CUDA SDK function to get the number of GPU cores
...
int report_image(char* file_name_output, struct detector_struct* detector_data, struct source_struct* source_data, float mean_energy_spectrum, unsigned long long int* image, double time_elapsed, unsigned long long int total_histories, int current_projection, int num_projections, double D_angle, double initial_angle, int myID, int numprocs)
{
  ...
  fprintf(file_ptr, "#     *****************************************************************************\n");
  fprintf(file_ptr, "#     ***                                                                       ***\n");
  fprintf(file_ptr, "#     ***         SOPPE, version 1.0 (https://github.com/VagnerSJ/SOPPE)        ***\n");
  fprintf(file_ptr, "#     ***                                                                       ***\n");
  fprintf(file_ptr, "#     *****************************************************************************\n");
  fprintf(file_ptr, "# \n");  
#ifdef USING_CUDA
  ...
  for(j=0; j<detector_data[0].num_pixels.y; j++)
  {
    for(i=0; i<detector_data[0].num_pixels.x; i++)
    {
      ...
      double total_energy_pixel = energy_noScatter + energy_compton + energy_rayleigh + energy_multiscatter;   // Find and report the pixel with maximum signal
...
//   printf("              (these material dose results will be equal to the materials dose tally below if the ROI covers all the voxels).\n");
  ...
  for(k=0; k<DZ; k++)
  {
    for(j=0; j<DY; j++)
    {
      for(i=0; i<DX; i++)
      {
        int voxel_geometry = (i+dose_ROI_x_min) + (j+dose_ROI_y_min)*voxel_data->num_voxels.x + (k+dose_ROI_z_min)*voxel_data->num_voxels.x*voxel_data->num_voxels.y;
        double inv_voxel_mass = 1.0 / (voxel_mat_dens[voxel_geometry].y*voxel_volume);

        int mat_number = (int)(voxel_mat_dens[voxel_geometry].x) - 1 ;  // Material number, starting at 0.
        ...
        double voxel_std_dev = (((double)voxels_Edep[voxel].y) * inv_N * inv_SCALE_eV * inv_voxel_mass - voxel_dose*voxel_dose) * inv_N;   // [sigma = (<Edep^2> - <Edep>^2) / N_hist]
  ...
  double voxel_mass_max_dose = voxel_volume*voxel_mat_dens[max_dose_voxel_geometry].y; 
