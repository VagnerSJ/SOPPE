...
#endif
{
  ...
  for( ; histories_per_thread>0; histories_per_thread--)
  {
    ...
    for(;;)
    {
      ...
      if (absvox<0)   // !!DeBuG!!  SOPPE_v1.0 ==> if I move this "if" above the code runs much slower!? Why???
      ...
      // *** Virtual interaction loop:  // New loop structure in SOPPE_v1.0: simulate all virtual events before sampling Compton & Rayleigh:  // !!DeBuG!!
...
//!  Electrons are not transported in SOPPE and therefore we are approximating
...
__device__
#endif
inline static void GCOa(float *energy, double *costh_Compton, int *mat, int2 *seed, struct compton_struct* cgco_SHARED)
{
